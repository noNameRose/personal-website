document.addEventListener("DOMContentLoaded", () => {
    ...
    const blockCotainers = document.querySelectorAll(".block-container");
blockCotainers.forEach((container) => {
...
function updateBlocksOpacity(blocks, order, isTop, progress) {
    blocks.forEach((block, idx) => {
        ...
        let opacity = isTop ? 1 - Math.min(1, Math.max(0, adjustedProgress))
